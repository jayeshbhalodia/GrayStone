...
class CI_Template {
    ...
    public function get_region($region, $file, $data=array(), $appned=true, $theme=false) {
        return $this->_region($val['region'], $val['file'], $val['data'], $val['append'], $val['theme'], true);
    }
     ...
     * @param type $theme 
     */
    private function _region($region, $file, $data=array(), $appned=true, $theme=false, $is_get_region=false) {
        if (array_key_exists($region, $this->template_ar['region'])) {
            ...
            if ($is_get_region) {
                $temp = $this->template_ar['region'][$region];
